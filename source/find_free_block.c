...
void	*best_fit(size_t size_to_be_alloc, t_zone *zone) { // size if the raw size
	...
	while (list_head != NULL \
		...
		&& left_over != 0)
	{
		if (*((size_t *)list_head - 1) >= padded(size_to_be_alloc) + MINIMUM_ALLOCATED_BLOCK_SIZE && left_over > *((size_t *)list_head -1) - padded(size_to_be_alloc) + MINIMUM_ALLOCATED_BLOCK_SIZE) {
